<script>
  ...
  function addHexagon(x, y, opts = {}) {
    ...
    hexagons.push({
      ...
      x,
      y,
      speed: opts.speed ?? Math.random() * HEXAGON_MAX_SPEED * 2 - HEXAGON_MAX_SPEED,
    });
  }
  ...
  function drawHexagonPath(ctx, hex) {
    ...
    ctx.lineTo(hex.x + Math.cos((Math.PI / 3) * (hex.sl + 1)) * HEXAGON_RADIUS, hex.y + Math.sin((Math.PI / 3) * (hex.sl + 1)) * HEXAGON_RADIUS);
    ctx.lineTo(hex.x + Math.cos((Math.PI / 3) * (hex.sl + 2)) * HEXAGON_RADIUS, hex.y + Math.sin((Math.PI / 3) * (hex.sl + 2)) * HEXAGON_RADIUS);
    ctx.lineTo(hex.x + Math.cos((Math.PI / 3) * (hex.sl + 3)) * HEXAGON_RADIUS, hex.y + Math.sin((Math.PI / 3) * (hex.sl + 3)) * HEXAGON_RADIUS);
    ctx.lineTo(
  ...
  function buildGrid() {
    hexagons = [];
    const hw = Math.ceil(canvas.width  / (1.5 * HEXAGON_RADIUS + HEXAGON_SPACING * 2)) + 1;
    const hh = Math.ceil(canvas.height / (s3p3 * HEXAGON_RADIUS + HEXAGON_SPACING * 2)) + 1;
    for (let x = 0; x < hw; x++) {
      for (let y = 0; y < hh; y++) {
        addHexagon(
          HEXAGON_RADIUS + HEXAGON_SPACING + (1.5 * HEXAGON_RADIUS + HEXAGON_SPACING * 2) * x,
          (s3p3 * HEXAGON_RADIUS) / 2 + HEXAGON_SPACING + (s3p3 * HEXAGON_RADIUS + HEXAGON_SPACING * 2) * y - (x % 2 ? (s3p3 * HEXAGON_RADIUS) / 2 : 0),
          { l: 0 },
        );
      }
    }
  ...
  function init() {
    canvas.width  = window.innerWidth;  /* ← viewport, not parentElement */
